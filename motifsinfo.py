...
def get_motif_filenames_and_motif_lengths(motif_locator_motifs_dir=default_motif_locator_motifs_dir,
                                          motif_locator_motifs_extension=default_motif_locator_motifs_extension):
    """
    Get motif filenames and motif lengths.
    ...
    :return: motif_id_to_filename_dict, motif_id_to_motif_length_dict
    """
...
class MotifsInfo:
    ...
    motif_id_to_filename_dict, motif_id_to_motif_length_dict = get_motif_filenames_and_motif_lengths(
    ...
    @staticmethod
    def write_motif_id_to_tfs_annotation_filename(motif_id_to_tfs_annotation_filename, separate_motifs_by='tab'):
        with open(motif_id_to_tfs_annotation_filename, 'w') as fh:
            if separate_motifs_by == 'tab':
                print('# motif ID\tTFs', file=fh)

                for motif_id in sorted(MotifsInfo.motif_to_tfs_dict):
                    print(motif_id, '\t'.join(sorted(MotifsInfo.motif_to_tfs_dict[motif_id])), sep='\t', file=fh)
            elif separate_motifs_by == 'line':
                print('# motif ID\tTF', file=fh)

                for motif_id in sorted(MotifsInfo.motif_to_tfs_dict):
                    for tf in MotifsInfo.motif_to_tfs_dict[motif_id]:
                        print(motif_id, tf, sep='\t', file=fh)
        ...
        with open(tf_to_motif_ids_annotation_filename, 'w') as fh:
            if separate_tfs_by == 'tab':
                ...
                for tf in sorted(MotifsInfo.tf_to_motifs_dict):
                    print(tf, '\t'.join(sorted(MotifsInfo.tf_to_motifs_dict[tf])), sep='\t', file=fh)
            elif separate_tfs_by == 'line':
                print('# TF\tmotif ID', file=fh)

                for tf in sorted(MotifsInfo.tf_to_motifs_dict):
                    for motif_id in MotifsInfo.tf_to_motifs_dict[tf]:
                        print(tf, motif_id, sep='\t', file=fh)


def main():
    motif_id_to_tfs_annotation_tab_filename = os.path.join(os.path.dirname(__file__),
                                                           'data',
                                                           'directly_annotated_motifs',
                                                           'motif_id_to_tfs_annotation.tab.tsv')
    motif_id_to_tfs_annotation_line_filename = os.path.join(os.path.dirname(__file__),
                                                            'data',
                                                            'directly_annotated_motifs',
                                                            'motif_id_to_tfs_annotation.line.tsv')
    tf_to_motif_ids_annotation_tab_filename = os.path.join(os.path.dirname(__file__),
                                                           'data',
                                                           'directly_annotated_motifs',
                                                           'tf_to_motif_ids_annotation.tab.tsv')
    tf_to_motif_ids_annotation_line_filename = os.path.join(os.path.dirname(__file__),
                                                            'data',
                                                            'directly_annotated_motifs',
                                                            'tf_to_motif_ids_annotation.line.tsv')

    print('Write motif ID to TFs association file "{0:s}" with each motif ID and all associated TFs on one line.'.format(
        motif_id_to_tfs_annotation_tab_filename)
    )
    MotifsInfo.write_motif_id_to_tfs_annotation_filename(
        motif_id_to_tfs_annotation_filename=motif_id_to_tfs_annotation_tab_filename,
        separate_motifs_by='tab'
    )

    print('Write motif ID to TFs association file "{0:s}" with each motif ID and TF pair on a separate line.'.format(
        motif_id_to_tfs_annotation_line_filename)
    )
    MotifsInfo.write_motif_id_to_tfs_annotation_filename(
        motif_id_to_tfs_annotation_filename=motif_id_to_tfs_annotation_line_filename,
